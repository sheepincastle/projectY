...
#이미지 불러오기
#0은 닫은 눈, 1은 열린 눈
img_list_0 = glob.glob('material data/ClosedFace/*.jpg')
img_list_1 = glob.glob('material data/OpenFace/*.jpg')
print('img0: ', len(img_list_0))
print('img1: ', len(img_list_1))

#학습할 데이터, 정답을 저장할 행렬 생성
eye_data = pandas.DataFrame(columns=['distance1', 'distance2', 'distance3', 'distance4', 'distance5', 'distance6', 'value'], index=[], dtype=float)

#학습할 데이터 및 정답 저장
#find_eyes함수의 결과로 나온 리스트의 항목이 6개 미만이라면, 즉 입력값의 일부가 없다면 continue를 통해 데이터를 저장하지 않고 다음 사진으로 넘어감
...
#결과값을 저장한 2차원 배열과 정답을 저장한 배열을 pickle파일 형태로 저장
